        ...
        <div className={`${isCollapsed ? 'w-20' : 'w-64'} dark:bg-[#020617] bg-[var(--bg-secondary)] border-r border-[var(--border-color)] flex flex-col flex-shrink-0 transition-all duration-300 relative`}>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-1 custom-scrollbar">
                ...
                <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-all flex items-center mb-4 ${activeView === 'dashboard' ? 'bg-indigo-600/10 text-indigo-400 border border-indigo-600/20 font-medium' : 'text-[var(--text-secondary)] hover:bg-[var(--bg-primary)] hover:text-[var(--text-primary)]'} ${isCollapsed ? 'justify-center px-0' : ''}`}
                    title={isCollapsed ? 'Dashboard' : ''}
                >
                    <span className="material-symbols-outlined text-[20px] mr-3">dataset</span>
                    ...
                    <div key={board.id} className="relative group">
                        {editingBoardId === board.id && !isCollapsed ? (
                            ...
                            <button
                                ...
                                className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-all flex items-center group ${activeView === 'board' && activeBoardId === board.id ? 'bg-indigo-600/10 text-indigo-400 border border-indigo-600/20' : 'text-[var(--text-secondary)] hover:bg-[var(--bg-primary)] hover:text-[var(--text-primary)]'} ${isCollapsed ? 'justify-center px-0' : 'justify-between'}`}
