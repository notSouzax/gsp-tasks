                            ...
                            <>
                                ...
                                {(column.default_reminder_enabled || tasks.some(t => t.reminder_enabled)) && (
                                    <button
                                        ...
                                        className={`p-1.5 rounded-lg transition-colors ${column.cardConfig?.auto_sort || column.card_config?.auto_sort ? '' : 'text-gray-400 hover:text-indigo-400 hover:bg-white/10'}`}
                                        style={column.cardConfig?.auto_sort || column.card_config?.auto_sort ? { color: borderColor, backgroundColor: borderColor + '1A' } : {}}
                                        title={column.cardConfig?.auto_sort || column.card_config?.auto_sort ? "Orden automático activado (Click para desactivar)" : "Activar orden automático por prioridad"}
                                    >
                                        <Icons.Sort />
                                    </button>
                                )}
