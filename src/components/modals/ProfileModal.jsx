...
const ProfileModal = ({ onClose }) => {
    ...
    const inputStyle = "w-full bg-slate-800/50 border border-slate-600/50 rounded-xl px-4 py-3 text-sm text-white outline-none focus:ring-2 focus:ring-indigo-500/40 focus:border-indigo-500/50 transition-all placeholder-slate-500";
    const labelStyle = "block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2";

    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div
                className="bg-[#0f172a] border border-slate-700/50 w-full max-w-lg rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in duration-200"
            ...
            >
                    ...
                    <div className="flex justify-between items-start">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="relative">
                                <img
                                    ...
                                    alt="Avatar"
                                    className="w-14 h-14 rounded-full object-cover border-2 border-indigo-500/30 shadow-lg shadow-indigo-500/10"
                                />
                                <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 rounded-full border-2 border-[#0f172a]"></div>
                            </div>
                            <div>
                                <h2 className="text-xl font-bold bg-gradient-to-r from-white via-indigo-200 to-indigo-400 bg-clip-text text-transparent">Mi Perfil</h2>
                        ...
                        <div className="flex items-center gap-1">
                            <button
                                ...
                                className="text-red-400 hover:text-red-300 hover:bg-red-400/10 px-3 py-1.5 rounded-lg text-xs font-semibold uppercase tracking-wider transition-colors"
                            >
                                Salir
                            </button>
                            <button onClick={onClose} className="text-slate-500 hover:text-slate-300 transition-colors p-1.5 hover:bg-slate-700/50 rounded-lg">
                ...
                {/* Tabs */}
                <div className="flex mx-6 border-b border-slate-700/50">
                    <button
                        onClick={() => setActiveTab('profile')}
                        className={`flex-1 px-4 py-2.5 text-xs font-semibold uppercase tracking-wider transition-colors ${activeTab === 'profile' ? 'text-indigo-400 border-b-2 border-indigo-500' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                        Datos Personales
                    </button>
                    <button
                        onClick={() => setActiveTab('security')}
                        className={`flex-1 px-4 py-2.5 text-xs font-semibold uppercase tracking-wider transition-colors ${activeTab === 'security' ? 'text-indigo-400 border-b-2 border-indigo-500' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                        Seguridad
                    </button>
                    <button
                        onClick={() => setActiveTab('workspace')}
                        className={`flex-1 px-4 py-2.5 text-xs font-semibold uppercase tracking-wider transition-colors ${activeTab === 'workspace' ? 'text-indigo-400 border-b-2 border-indigo-500' : 'text-slate-500 hover:text-slate-300'}`}
                    >
                        Workspace
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
                    ...
                    {/* Message */}
                    {message.text && (
                        <div className={`p-3 rounded-lg text-sm ${message.type === 'success' ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
                    ...
                    {activeTab === 'profile' && (
                        <form onSubmit={handleSaveProfile} className="space-y-5">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex-1">
                                        <label className="flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-600/50 text-slate-300 rounded-lg text-sm font-medium transition-all cursor-pointer">
                                            <input
                                                type="file"
                                                accept="image/*"
                                        ...
                                        <p className="text-xs text-slate-600 mt-1.5">JPG, PNG o GIF. Máximo 2MB.</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <button
                                type="submit"
                                disabled={saving}
                                className="w-full px-4 py-3 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 text-white rounded-xl text-sm font-bold transition-all disabled:opacity-50 shadow-lg shadow-indigo-500/20 hover:-translate-y-0.5"
                            >
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        ...
                                        className="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-600/50 text-slate-300 rounded-lg text-sm font-semibold transition-all disabled:opacity-50"
                                    >
                                        Cambiar
                                    </button>
                                </div>
                                <p className="text-xs text-slate-600">Recibirás un email de confirmación</p>
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <button
                                    ...
                                    className="w-full px-4 py-3 bg-gradient-to-r from-orange-600 to-amber-500 hover:from-orange-500 hover:to-amber-400 text-white rounded-xl text-sm font-bold transition-all disabled:opacity-50 shadow-lg shadow-orange-500/20 hover:-translate-y-0.5"
                                >
